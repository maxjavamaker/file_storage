...
public class DocumentStoreImpl implements DocumentStore {
    private HashMap<URI, Document> documents = new HashMap<>();
    ...
    public int put(InputStream input, URI uri, DocumentFormat format) throws IOException{
        ...
        if (input == null){
            if (documents.containsKey(uri)) {
                return documents.remove(uri).hashCode();
            }
            return 0;
        }
    ...
    public boolean delete(URI url){
        return documents.remove(url) != null;
    }

    private int add(URI uri, Document document){
        if (documents.containsKey(uri)){
            return documents.replace(uri, document).hashCode();
        }
